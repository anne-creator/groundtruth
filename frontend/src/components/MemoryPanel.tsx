      ...
      {open && (
        <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', padding: '10px 12px', gap: 10 }}>
          {!session?.session_token && session?.status !== 'idle' && (
            <div style={{ fontSize: 11, color: '#fbbf24', background: '#422006', borderRadius: 6, padding: 8 }}>
              Session token missing — migrate DB or redeploy `/start-session` with `session_token` column.
            </div>
          )}

          {unavailable && (
            <div style={{ fontSize: 11, color: '#fcd34d', background: '#422006', borderRadius: 6, padding: 8 }}>
            ...
            {(['profile', 'evidence'] as const).map((t) => (
              <button
                ...
                style={{
                  ...
                  padding: '6px 0',
                  borderRadius: 6,
                  border: tab === t ? '1px solid #60a5fa' : '1px solid #334155',
                  background: tab === t ? '#1e3a8a' : '#1e293b',
                  color: '#e2e8f0',
                  fontWeight: 600,
                  cursor: 'pointer',
                  fontSize: 11,
          ...
          {!loading && hint && !unavailable && (
            <div style={{ fontSize: 11, color: '#93c5fd' }}>{hint}</div>
          )}
          ...
          {tab === 'profile' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {AGENTS.map((id) => {
                ...
                const dy = pf.dynamic ?? [];
                return (
                  <div key={id} style={{ border: '1px solid #334155', borderRadius: 8, padding: 10 }}>
                    <div style={{ fontSize: 12, fontWeight: 700, marginBottom: 6, color: '#94a3b8' }}>
                      {AGENT_LABELS[id as AgentId]}
                    </div>
                    <div style={{ fontSize: 11, color: '#64748b', marginBottom: 4 }}>Static</div>
                    <pre style={{
                      whiteSpace: 'pre-wrap',
                      margin: '0 0 8px 0',
                      fontSize: 11,
                    ...
                    <div style={{ fontSize: 11, color: '#64748b', marginBottom: 4 }}>Dynamic</div>
                    <pre style={{
                      whiteSpace: 'pre-wrap',
                      margin: 0,
                      fontSize: 11,
          ...
          {tab === 'evidence' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <label style={{ fontSize: 11, color: '#94a3b8' }}>
                ...
                <input type="file" onChange={(e) => void onFile(e)} style={{ display: 'block', marginTop: 6, fontSize: 11, color: '#cbd5e1' }} />
              </label>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                ...
                {(payload?.processing?.documents?.length ?? 0) === 0 && (
                  <span style={{ fontSize: 11, color: '#64748b' }}>No uploads processing</span>
                )}
              </div>
              ...
              <ul style={{ margin: 0, paddingLeft: 16, color: '#cbd5e1', fontSize: 11 }}>
                {(payload?.evidenceList?.memories ?? []).slice(0, 20).map((m) => (
                  <li key={m.id} style={{ marginBottom: 4 }}>
                    {m.title ?? m.customId ?? m.id}: <span style={{ color: '#64748b' }}>{m.status}</span>
                  </li>
                ))}
              </ul>
              ...
              {Boolean(evidenceQ.trim()) && (
                <>
                  ...
                  <ul style={{ margin: 0, paddingLeft: 16, color: '#e2e8f0', fontSize: 11 }}>
                    {(payload?.evidenceSearch?.results ?? []).slice(0, 8).map((r, i) => {
                      ...
                      <li key={r.documentId ?? i} style={{ marginBottom: 6 }}>
                        <div style={{ color: '#94a3b8' }}>{r.title ?? r.documentId}</div>
                        {preview ? (
                          <div style={{ color: '#64748b', fontSize: 10 }}>{preview}</div>
                        ) : null}
                      </li>
                      );
                    })}
